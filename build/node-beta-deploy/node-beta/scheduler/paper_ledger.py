    ...
    try:
        ...
        for sig in signals:
            ...
            if not existing:
                ...
                try:
                    hist = yf.Ticker(sig.ticker + ".NS" if not sig.ticker.startswith("^") else sig.ticker).history(period="1d")
        ...
        for pos in open_positions:
            try:
                hist = yf.Ticker(pos.ticker + ".NS" if not pos.ticker.startswith("^") else pos.ticker).history(period="1d")
        ...
        session.commit()
        logger.info(f"Recorded End-of-Day Portfolio Snapshot: {total_capital} capital, {unrealized_pnl} uPNL")
