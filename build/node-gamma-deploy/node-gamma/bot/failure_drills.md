...
- **Action**: Stop the ollama service on node-alpha.
- **Expected Outcome**:
...
- **Action**: Power off or completely disconnect node-alpha midway through daily inference.
- **Expected Outcome**:
  - Watchdog avoids spamming HIGH Celery/Ollama alerts, as ping timeouts silently suppress child alerts.
  - node-beta enqueuing tasks will pile up in the Redis broker.
  - Upon node-alpha recovery, tasks resume draining from Redis broker where they left off.
